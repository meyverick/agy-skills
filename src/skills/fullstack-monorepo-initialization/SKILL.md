...
*   **Local Setup & Repository**: Provide the exact terminal shell scripts utilizing the GitHub CLI (`gh`) to initialize Git local to the workspace, set `main` as the primary branch, and create a remote private GitHub repository named exactly after the workspace folder. Provide a template `infra.config.json` file to cleanly map Encore's database needs to a self-hosted PostgreSQL database container in CapRover.
