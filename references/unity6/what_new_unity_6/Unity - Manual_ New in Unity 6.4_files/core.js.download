...
$(window).on("load", function(){
    ...
    /**
     * Recursive loop to build the TOC
     * @param links an array of link objects defined in toc.js generated by HtmlGenerator.cs
     * @param ul the root of the table of contents
     */
    function loop(links, ul) {
        $.each(links, function (key, obj) {
            ...
            // Compare full relative paths, not just filenames, to avoid
            // collisions between pages with the same name in different
            // directories (e.g. "get-started" vs "adaptive-performance/get-started"):
            var $pagePath = location.pathname.replace(/\.html$/, '');
            var $suffix = '/' + obj['link'];
            var $class = ($pagePath.slice(-$suffix.length) === $suffix) ? 'current' : '';

            // If this item is an organizational item (no link), create a span,
            // otherwise create an anchor element linking to the specified page:
            if (obj['link'] == 'null') {
                $li.text(obj['title']).addClass('nl').wrapInner('<span tabindex="0" role="button"></span>');
            } else {
            ...
            // if this is a branch node recursively create descendant links,
            // otherwise mark it as a leaf node which is styled differently:
            if (obj['children'] && obj['children'].length > 0) {
                $li.prepend('<div class="arrow" tabindex="0" role="button"></div>');
    ...
    // the toc object is defined in toc.js generated by HtmlGenerator.cs:
    var data = toc;

    // root element of the table of contents:
    var ul = $('<ul/>');
    ...
    // If the current page is not in the table of contents
    // find a sensible default page to set as current:
    if ($('.sidebar-menu .toc .current').length == 0) {
        ...
        // try and match a parent page in the table of contents by using the URL,
        // otherwise fallback to matching the full URL:
    ...
    // expand chidren of current section in sidebar
    var childList = $('.sidebar-menu .toc .current').parents('li').children('ul').first();
    childList.show();

    // update the styling of the current section to show that it's expanded:
    var arrow = $('.sidebar-menu .toc .current').parents('li').children('div').first();
    arrow.removeClass('collapsed');
    arrow.addClass('expanded');

    /**
     * add interactivity to the toc items:
     * @param arrow the arrow icon that indicates expanded/collapsed state
     */
    function toggleArrowAndList(arrow) {
        var $arrow = $(arrow);
    ...
    // Clicks on arrows
    var $arrows = $('.sidebar-menu .toc li:has(ul) .arrow');
    $arrows.on('click', function () {
        toggleArrowAndList(this);
    });

    // Keyboard on arrows (Space/Enter)
    $arrows.on('keydown', function (e) {
        var isSpace = e.key === ' ' || e.key === 'Spacebar' || e.which === 32 || e.keyCode === 32;
    ...
    // Clicks on organizational "no-link" spans:
    var $noLinkSpans = $('.sidebar-menu .toc .nl span');
    const noLinkSpanHandler = function (e) {
        var $arrow = $(this).prev('.arrow');
        if ($arrow.length) toggleArrowAndList($arrow);
    }

    // clicks on the spans:
    $noLinkSpans.on('click', noLinkSpanHandler);

    // Keyboard on organizational spans (Space/Enter):
    $noLinkSpans.on('keydown', function (e) {
        var isSpace = e.key === ' ' || e.key === 'Spacebar' || e.which === 32 || e.keyCode === 32;
        var isEnter = e.key === 'Enter' || e.which === 13 || e.keyCode === 13;

        if (isSpace || isEnter) {
            e.preventDefault();
            noLinkSpanHandler.call(this);
        }
    });
});
